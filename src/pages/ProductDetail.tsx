...
const ProductDetail = () => {
  ...
  const productSchema = {
    ...
    name: product.description ?? product.sku,
    sku: product.sku,
    url: `https://daneauto.ca/product/${encodeURIComponent(product.sku)}`,
    ...(product.partslink_number ? { mpn: product.partslink_number } : {}),
    ...(product.oem_number ? { gtin8: product.oem_number } : {}),
    ...(product.image_url ? { image: product.image_url } : {}),
    description: [
      product.description ?? product.sku,
      `Fits ${product.year} ${product.make} ${product.model}`,
      product.certification ? `${product.certification} certified` : null,
    ].filter(Boolean).join(". ") + ".",
    ...
    offers: {
      ...
      ...(product.list_price && hasDiscount ? { priceValidUntil: new Date(Date.now() + 30 * 86400000).toISOString().split("T")[0] } : {}),
    },
    isRelatedTo: {
      "@type": "Vehicle",
